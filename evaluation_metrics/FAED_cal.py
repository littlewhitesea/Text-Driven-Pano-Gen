# The following code is modified from FAED.py (https://github.com/chengzhag/PanFusion/tree/main/models/faed).
...
from utils import WandbLightningModule
...
def load_images_from_folder(folder_path, device, batch_size=50):
    """Load images from a folder, preprocess them, and return them in batches."""
    """The choice of batch size matches the original FID score implementation."""
    images = []
    for image_path in Path(folder_path).rglob("*.*"):  # Read all files in the folder
        img = read_image(str(image_path)).to(device)  # Load image and move to device
        if img.shape[0] != 3:  # Ensure 3 channels
            img = img.expand(3, -1, -1)
        # img = transforms.Resize((512, 512))(img)  # Resize to match FID requirements
        images.append(img)

    # Stack and return images in batches
    stacked_images = torch.stack(images)
    return [stacked_images[i:i + batch_size] for i in range(0, len(stacked_images), batch_size)]
...
if __name__ == "__main__":
    ...
    real_batches = load_images_from_folder(real_folder, device)
    fake_batches = load_images_from_folder(fake_folder, device)

    # Initialize metric
    fid_metric = FrechetAutoEncoderDistance(pano_height=512).to(device)
